  ...
  return (
    ...
    >
        ...
        <div className="relative w-full bg-black" style={{ aspectRatio: '16/9' }}>
          {showVideo ? (
            ...
            <button
              ...
              className="w-full h-full flex flex-col items-center justify-center gap-3 group/play"
              style={{ background: `linear-gradient(135deg, ${project.color}12, rgba(15,16,20,0.95))` }}
            >
              <div
                className="flex items-center justify-center w-14 h-14 rounded-full transition-transform duration-200 group-hover/play:scale-110"
                style={{
              ...
              >
                <Play size={22} fill={project.color} style={{ color: project.color, marginLeft: 3 }} />
              </div>
              <span className="font-orbitron text-xs tracking-widest" style={{ color: project.color }}>
                WATCH DEMO
              </span>
            </button>
          )}
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        >
          {String(i + 1).padStart(2, '0')} / {String(project.total).padStart(2, '0')}
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((project, i) => (
            <ProjectCard key={i} project={{ ...project, total: projects.length }} i={i} />
          ))}
        </div>
      </div>
    </section>
